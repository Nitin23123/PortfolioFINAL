...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    const enterHandler = e => {
      ...
      activeTarget = target;
      Array.from(cornersRef.current).forEach(c => gsap.killTweensOf(c));
      gsap.killTweensOf(cursorRef.current, 'rotation');
      spinTl.current?.pause();
      gsap.set(cursorRef.current, { rotation: 0 });
      gsap.to(cursorRef.current, { opacity: 1, duration: 0.3, ease: 'power3.out' });
      ...
      const leaveHandler = () => {
        gsap.to(cursorRef.current, { opacity: 0, duration: 0.25, ease: 'power3.inOut' });
